...
import { deleteImageUrl, pauseImageUrl, playImageUrl } from "../helper/imageLink";
const Popup = () => {
    ...
    const handlePlayPause = (index: number) => {
        ...
        chrome.runtime.sendMessage({type:"playPause",url:bookmarks[index].url});
    }
    return (
            ...
            <div className="flex justify-center items-center h-full bg-slate-100 w-full p-1 rounded-sm border-[1px] border-gray-300">
                {
                    bookmarks && bookmarks.length > 0 ?
                                ...
                                <tbody>
                                    {bookmarks?.map((bookmark: localStorageDataInterface, index: number) =>
                                        <tr className="bg-white dark:bg-gray-800" key={index}>
                                            <th scope="row" className="px-3 py-1.5 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                {`BookMark-${index + 1}`}
                                            </th>
                                            ...
                                            <td className="px-3 py-1.5 w-full flex justify-center">
                                                {
                                                    activeBookMarkNumber === index ? <img src={pauseImageUrl} className="w-5 h-5 cursor-pointer" onClick={() => setActiveBookMarkNumber(-1)} /> : <img src={playImageUrl} className="w-5 h-5 cursor-pointer" onClick={() => handlePlayPause(index)} />
                                                }
                                            </td>
                                            <td className="px-3 py-1.5">
                                                <img src={deleteImageUrl} className="w-5 h-5 cursor-pointer" />
                                            </td>
                                        </tr>
                                    )}
                        ...
                        : <h2 className="flex justify-center items-center h-full ">No bookmarks yet</h2>
                }
            </div>
        </div>
    )
